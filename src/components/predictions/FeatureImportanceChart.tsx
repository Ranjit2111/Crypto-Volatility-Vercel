...
export function FeatureImportanceChart() {
  ...
  const [refreshKey, setRefreshKey] = useState(0); // Used to force image reload
  ...
  useEffect(() => {
    ...
    try {
      ...
      setChartUrl(`${url}?key=${refreshKey}`); // Add cache-busting query param
    } catch (error) {
      console.error("Error fetching feature importance plot URL:", error);
      setHasError(true);
      setIsLoading(false);
    }
  }, [refreshKey]);
  ...
  const handleDownload = () => {
    if (!chartUrl) return;
    // Remove cache-busting param for download
              ...
              {isLoading && !hasError ? (
                ...
                >
                  <Image
                    ...
                    layout="fill"
                    objectFit="contain"
                    unoptimized // if the image is dynamically generated and not optimizable by Next.js
